...
const AMSAuditForm: React.FC<AMSAuditFormProps> = ({ isOpen, onClose, initialData }) => {
  ...
  useEffect(() => {
      ...
      const currentDrug = abx[activeAbxIndex];
      if (!currentDrug.drug) return;

      const runChecks = async () => {
          let results: any = { ...aiAnalysis[activeAbxIndex] };
          ...
          if (hasChanges) {
              setAiAnalysis(prev => ({ ...prev, [activeAbxIndex]: results }));
          }
      };
      ...
      activeAbxIndex, 
      abx[activeAbxIndex]?.drug, 
      abx[activeAbxIndex]?.dose, 
      abx[activeAbxIndex]?.freq, 
      patient.egfr, 
      patient.weight, 
      patientMode
  ]);
